     ...
     if(isset($_SESSION['login_user']))
     {
      ...
      <div class="b">
        <form class="" action="" method="post">
          <button name="submit2" type="submit" id="btn1" class="btn ">RETURNED</button>
          <button name="submit3" type="submit" id="btn2" class="btn ">EXPIRED</button>
        </form>
      </div>
     ...
     $c=0;
        ...
        if(isset($_SESSION['login_user']))
        {
          ...
          if(isset($_POST['submit2']))
          {
            ...
            INNER JOIN books ON issue_book.bid=books.bid WHERE issue_book.approve = '$ret'
          ...
          else if(isset($_POST['submit3']))
          {
            $sql = "SELECT student.username, roll, books.bid, name, authors, edition,approve, issue,
            issue_book.return FROM student INNER JOIN issue_book ON student.username=issue_book.username
            INNER JOIN books ON issue_book.bid=books.bid WHERE issue_book.approve = '$exp'
             ORDER BY `issue_book`.`return` DESC";
            ...
            $sql = "SELECT student.username, roll, books.bid, name, authors, edition,approve, issue,
            issue_book.return FROM student INNER JOIN issue_book ON student.username=issue_book.username
            INNER JOIN books ON issue_book.bid=books.bid WHERE issue_book.approve != '' AND
            issue_book.approve != 'YES' ORDER BY `issue_book`.`return` DESC";
